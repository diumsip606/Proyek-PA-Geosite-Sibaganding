        ...
        <table class="table table-custom">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($item->gambar)
                            <img src="{{ asset('storage/' . $item->gambar) }}" class="preview-img" alt="Gambar Galeri">
                        @else
                            <div class="bg-secondary text-white text-center rounded d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;">
                    ...
                    <td>
                        ...
                        @if($item->is_hero)
                            <span class="badge bg-warning text-dark ms-1"><i class="fas fa-star"></i> Hero</span>
                        @endif
                    </td>
                    <td>
                        <span class="badge bg-info text-white">{{ $item->kategori->nama ?? 'Tanpa Kategori' }}</span>
                    </td>
                    <td>{{ $item->lokasi ?: '-' }}</td>
                    <td>
                        @if($item->status)
                        ...
                        <div class="d-flex gap-1 align-items-center">
                            ...
                            @if(!$item->is_hero)
                                <form action="{{ route('admin.galeri.set_hero', $item->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-outline-custom" style="border-color: #3b82f6; color: #3b82f6;" title="Jadikan Hero" onclick="return confirm('Jadikan hero?')">
                            ...
                            @else
                                <form action="{{ route('admin.galeri.unset_hero', $item->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-outline-custom text-warning" style="border-color: #ffc107; color: #ffc107;" title="Batalkan Hero" onclick="return confirm('Batalkan hero?')">
                ...
                @empty
                <tr><td colspan="7" class="text-center py-4">Belum ada data galeri</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
