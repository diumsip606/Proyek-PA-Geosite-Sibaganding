...
<div class="d-flex align-items-center mb-3">
    ...
    <h5 class="mb-0">Edit Info Kontak</h5>
</div>
...
<div class="form-card" style="background: white; border-radius: 16px; padding: 30px; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
    ...
    <div class="row">
        <!-- FORM UTAMA -->
        <div class="col-lg-8">
            <form action="{{ route('admin.kontak-info.update', $kontakInfo->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="tipe" class="form-select" required id="tipeSelect" style="padding: 12px; border-radius: 8px;">
                            ...
                            <option value="alamat" {{ old('tipe', $kontakInfo->tipe) == 'alamat' ? 'selected' : '' }}>Alamat</option>
                            <option value="telepon" {{ old('tipe', $kontakInfo->tipe) == 'telepon' ? 'selected' : '' }}>Telepon</option>
                            <option value="email" {{ old('tipe', $kontakInfo->tipe) == 'email' ? 'selected' : '' }}>Email</option>
                            <option value="sosial_media" {{ old('tipe', $kontakInfo->tipe) == 'sosial_media' ? 'selected' : '' }}>Sosial Media</option>
                            <option value="jam_operasional" {{ old('tipe', $kontakInfo->tipe) == 'jam_operasional' ? 'selected' : '' }}>Jam Operasional</option>
                        </select>
                        <small class="text-muted">Pilih kategori informasi yang ingin Anda ubah.</small>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold" id="labelName">Label / Keterangan <small class="text-muted">(opsional)</small></label>
                        <input type="text" name="label" class="form-control" value="{{ old('label', $kontakInfo->label) }}" placeholder="Contoh: Kantor Utama, Instagram, Senin - Jumat" id="labelInput" style="padding: 12px; border-radius: 8px;">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" name="nilai" class="form-control" value="{{ old('nilai', $kontakInfo->nilai) }}" placeholder="Contoh: +62 812 3456 7890, info@geotoba.com" required id="nilaiInput" style="padding: 12px; border-radius: 8px;">
                        <small class="text-muted d-block mt-1" id="nilaiHint"></small>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Urutan Tampilan</label>
                        <input type="number" name="urutan" class="form-control" value="{{ old('urutan', $kontakInfo->urutan) }}" min="0" style="padding: 12px; border-radius: 8px;">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" name="is_active" value="1" id="isActive" {{ old('is_active', $kontakInfo->is_active) ? 'checked' : '' }}>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-primary px-4" style="border-radius: 8px; padding: 10px 20px;">Update Informasi</button>
